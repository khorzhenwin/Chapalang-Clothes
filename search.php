...
(
  ...
  WHERE product.ProductName LIKE '%$q_esc%' 
  OR 
  product.ProductDescription LIKE '%$q_esc%' 
  OR 
  product.ProductTags LIKE '%$q_esc%' 
  GROUP BY product.ProductId 
  ORDER BY `TotalSold` DESC
) resultTable 
  WHERE resultTable.ProductStatus='Approved' " . $minmax_query . $star_query . $sortby_query . $limitQuery;

$rows = mysqlidb::fetchAllRows($fullQuery);

$numrows = mysqlidb::fetchRow("SELECT COUNT(*) as NumRows FROM 
(
  SELECT product.*,IFNULL(AVG(review.ReviewRating),0) as ProductRating,
  IFNULL(SUM(orderitem.ProductQuantity),0) as TotalSold 
  FROM product 
  ...
  WHERE resultTable.ProductStatus='Approved' " . $minmax_query . $star_query . $sortby_query)["NumRows"];
